...
doc.search('.rates-table-wrapper .table-row').each_with_index do |element, index|
  ...
  p bank = Bank.create!(name:bank_name, image_url:image.attr('src').to_s)
  p bank.rates.create!(interest_value:one_year)
  p bank.rates.create!(interest_value:two_years)
  p bank.rates.create!(interest_value:three_years)
end
...
# anz.save!
# p "Created ANZ Bank"
# rate_1 = Rate.create!(interest_value: 0.069, bank_id: anz.id)
# rate_1.save!
# p "Created rate 1"
